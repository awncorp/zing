...
SKIP: {
  ...
  $subs->example(-1, 'flow', 'method', fun($tryable) {
    ...
    my $step_0 = $result;
    is_deeply $step_0->name, 'on_reset';

    my $step_1 = $step_0->next;
    is_deeply $step_1->name, 'on_register';

    my $step_2 = $step_1->next;
    is_deeply $step_2->name, 'on_perform';

    my $step_3 = $step_2->next;
    is_deeply $step_3->name, 'on_receive';

    my $step_4 = $step_3->next;
    is_deeply $step_4->name, 'on_suicide';
